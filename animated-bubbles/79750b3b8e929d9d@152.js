// https://observablehq.com/@uwdata/arquero@152
...
md`Import Arquero and extend it to print HTML tables in Observable:`
  ...
  main.variable(observer("aq")).define("aq", ["require","toView"], async function(require,toView)
{
  const aq = await require('arquero@2.2.0');

  // extend table prototype to provide an HTML table view
  aq.internal.ColumnTable.prototype.view = toView;
...
`
  ...
  main.variable(observer("toView")).define("toView", ["width","html"], function(width,html)
{
  ...
  const cellStyle = 'padding: 1px 5px; white-space: nowrap; overflow-x: hidden; text-overflow: ellipsis;';
  ...
  return function(opt = {}) {
    ...
    if (typeof opt.color === 'function') {
      ...
      this.columnNames().forEach(name => color[name] = opt.color);
    } else {
    ...
    const size = `max-width: ${+opt.width || width}px; max-height: ${+opt.height || 270}px`;
    const style = `${size}; overflow-x: auto; overflow-y: auto;`;
    const view = html`<div style="${style}">${this.toHTML(opt)}</div>`;
    view.value = this;
    return view;
  };
}
